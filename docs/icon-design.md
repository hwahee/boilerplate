...
### 4.1 공개 어휘는 "역할"로 명명한다

`chevron-down`(생김새)이 아니라 `disclosure.expand`(역할)로 명명합니다. 이유:

- 생김새로 명명하면 **office 스킨이 ▼(채워진 삼각형)을 쓰고 싶을 때 이름이 거짓말**이 됩니다.
- 역할 기반 이름은 §3.1의 명명 철학 차이를 호출부로 새어나오지 않게 막습니다.
- i18n의 `MessageKey`가 문자열 내용이 아니라 용처로 명명된 것과 같은 원리입니다.

**입도(granularity) 결정:** 현재 `ChevronDown`은 두 곳(아코디언 확장, 팔레트 드롭다운
트리거)에서 쓰입니다. **이 둘은 나눠야 합니다** — office 스킨에서 콤보박스 화살표(▼)와
그룹 확장 표시(＋/－)는 실제로 다른 글리프이기 때문입니다. 반대로 지금 하나로 충분한 것을
미리 쪼개지 말 것. _"스킨 하나라도 다르게 그리고 싶어지면 그때 쪼갠다"_ 가 규칙입니다.
