...
실시간 멀티유저 데모 기능입니다. 로비(`/rooms`)에서 닉네임을 정하고 방을 만들어 입장하면:

- **실시간 채팅** — 입장/퇴장 시스템 메시지, 닉네임별 고정 색상
- **공유 그림판** — 붓 색상/굵기/지우개, 스트로크가 청크 단위로 스트리밍되어 모든
  참가자에게 실시간 동기화되고, 새 입장자는 스냅샷으로 따라잡습니다
- **그림 퀴즈 (캐치마인드)** — 출제자에게만 단어(한/영)가 공개되고, 나머지는 채팅으로
  정답을 맞히며, 닉네임 기준 점수판이 유지됩니다
- **주사위 (D6/D20/D100)** — 서버가 굴려서 채팅에 결과를 공개합니다
- **이모지 리액션** — 그림판 위로 떠오르는 리액션을 던집니다

구조는 저장소 컨벤션을 그대로 따릅니다: 도메인 계약과 WS 프로토콜은
`src/shared/rooms`(인바운드 메시지는 검증 파사드로 파싱), 로비 REST API는 `/api/rooms`,
방 내부의 모든 활동은 `/ws/rooms/:roomId` WebSocket으로 오갑니다. 방은 영속되지 않는
**라이브 세션**이라 상태(`src/server/services/live-room-service.ts`)는 프로세스
메모리에만 존재합니다 — 다중 인스턴스로 확장하려면 sticky session 또는 공유 저장소 +
pub/sub 팬아웃이 필요합니다(서비스에 문서화된 한계).

```
src/
├── shared/          # 서버·클라이언트 공통 (양쪽 모두에서 import 가능)
│   ├── rooms/       # ★ 놀이방 도메인 + WS 프로토콜 + 퀴즈 단어 (공용 계약)
