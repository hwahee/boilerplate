...
describe('listTodosQueryValidator', () => {
  test('applies the pagination defaults', () => {
  ...
  test('accepts filters and sorting', () => {
    expect(
    ...
    ).toMatchObject({ page: 3, pageSize: 50, sortBy: 'title', status: 'open', q: 'milk' });
  });

  test('rejects out-of-range and unknown values', () => {
    expect(listTodosQueryValidator.safeParse({ page: 0 }).ok).toBe(false);
    expect(listTodosQueryValidator.safeParse({ pageSize: 101 }).ok).toBe(false);
